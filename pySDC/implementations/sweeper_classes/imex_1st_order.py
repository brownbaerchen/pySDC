...
class imex_1st_order_artificial_viscosity(imex_1st_order):
    ...
    def __init__(self, params, level):
        ...
        self.QA = self.get_Qdelta_implicit(qd_type=self.params.QA)
    ...
    def update_nodes(self):
        ...
        for m in range(M):
            ...
            for j in range(1, M + 1):
                integral[m] -= L.dt * (
                    self.QI[m + 1, j] * L.f[j].impl
                    + self.QA[m + 1, j] * L.f[j].artificial
                    + self.QE[m + 1, j] * L.f[j].expl
                )
            # add initial value
            integral[m] += L.u[0]
            # add tau if associated
