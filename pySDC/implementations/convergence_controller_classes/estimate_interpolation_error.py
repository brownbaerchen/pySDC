...
class EstimateInterpolationErrorEndpoint(ConvergenceController):
    ...
    def post_iteration_processing(self, controller, S, **kwargs):
        ...
        if self.check_convergence(S):
            ...
            print(coll.quad_type, coll.nodes)

            new_coll = self.CollBase(
                **{**self.params.sweeper_params, 'quad_type': 'RADAU-RIGHT' if coll.quad_type == 'GAUSS' else 'GAUSS'}
            )
            print(new_coll.quad_type, new_coll.nodes)

            interpolator = LagrangeApproximation(points=np.append(0, coll.nodes))
            interpolation_matrix = interpolator.getInterpolationMatrix(np.append(0, new_coll.nodes))
            u = [L.u[i].flatten() if L.u[i] is not None else L.u[i] for i in range(coll.num_nodes + 1)]
            u_inter = [me.reshape(L.prob.init[0]) for me in self.matmul(interpolation_matrix, u)]

            if coll.quad_type == 'GAUSS':
            ...
            if self.comm:
                buf = np.array(abs(u_end_high - u_end_low) if self.comm.rank == estimate_on_node - 1 else 0.0)
                self.comm.Bcast(buf, root=estimate_on_node - 1)
                L.status.error_embedded_estimate = buf
            else:
                L.status.error_embedded_estimate = abs(u_end_high - u_end_low)
