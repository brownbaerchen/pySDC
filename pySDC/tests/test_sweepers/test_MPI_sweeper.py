...
def individual_test(num_nodes, quad_type, residual_type, imex, initGuess, useNCCL, launch=True):
    ...
    if launch:
        ...
        if useNCCL:
            mpi_command = f'srun -n {num_nodes}'
        else:
            mpi_command = f'mpirun -np {num_nodes}'
        cmd = f"{mpi_command} python {__file__} --test_sweeper {num_nodes} {quad_type} {residual_type} {imex} {initGuess} {useNCCL}".split()
