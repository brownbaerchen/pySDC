...
def plot_stability(stats, ax=None, iter=None):
    ...
    u = get_sorted(stats, type="u", sortby="iter", time=2.0)
    # print(u)
    ...
    for i in iter:
        ...
        U = np.reshape(
            [me[1] for me in u if me[0] == i],
            (len(np.unique(lambdas.real)), len(np.unique(lambdas.imag))),
        )
...
if __name__ == "__main__":
    custom_description = None
    stats, controller, Tend = run_dahlquist(
        custom_description=custom_description, Tend=2.0
    )
