...
def analyse_resilience():  # pragma: no cover
    ...
    ###########
    bit_zero = stats_analyser.get_mask(key='bit', op='eq', val=0)
    # stats_analyser.print_faults(mask=bit_zero)
    stats_analyser.scrutinize_visual(BaseStrategy(), run=188, faults=True)
    plt.show()
    ###########
