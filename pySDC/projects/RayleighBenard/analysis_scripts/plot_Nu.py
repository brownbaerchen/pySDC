...
def plot_Nu_over_time_Ra1e8():  # pragma: no cover
    Nu_fig, Nu_axs = plt.subplots(4, 1, sharex=True, sharey=True, figsize=figsize(scale=1, ratio=1.4))

    res = 256
    converged_from = 8
    Nu_axs[1].axvline(converged_from)
    Nu_axs[1].axhline(30)
    ...
    plot_Nu(res, [0.0007, 0.0005], 'RBC3DG4R4RKRa1e8', ref_data, Nu_axs[2], 'RK443', converged_from)
    ...
    Nu_fig.tight_layout()
    Nu_fig.savefig('./plots/Nu_over_time_Ra1e8.pdf', bbox_inches='tight')


def plot_Nu_over_time_Ra1e9():  # pragma: no cover
    Nu_fig, Nu_axs = plt.subplots(4, 1, sharex=True, sharey=True, figsize=figsize(scale=1, ratio=1.4))
    ...
    ref_data = get_pySDC_data(res=res, dt=0.0005, config_name='RBC3DG4R4SDC44Ra1e9')

    plot_Nu(res, [0.0005], 'RBC3DG4R4SDC44Ra1e9', ref_data, Nu_axs[0], 'SDC44', converged_from)
    plot_Nu(res, [0.0003, 0.0001], 'RBC3DG4R4SDC23Ra1e9', ref_data, Nu_axs[1], 'SDC23', converged_from)
    plot_Nu(res, [0.0003, 0.0001], 'RBC3DG4R4RKRa1e9', ref_data, Nu_axs[2], 'RK443', converged_from)
    plot_Nu(res, [0.00005], 'RBC3DG4R4EulerRa1e9', ref_data, Nu_axs[3], 'RK111', converged_from)

    Nu_axs[-1].set_xlabel('$t$')
    Nu_axs[-1].set_ylabel('$Nu$')

    Nu_fig.tight_layout()
    Nu_fig.savefig('./plots/Nu_over_time_Ra1e9.pdf', bbox_inches='tight')
