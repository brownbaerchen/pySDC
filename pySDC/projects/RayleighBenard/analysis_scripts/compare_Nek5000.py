...
def get_Nek5000_Data(Ra, base_path='data/Nek5000'):
    assert type(Ra) == str
...
def plot_Re_scaling(ax):  # pragma: no cover

    # reference values
    for Ra in ['1e5', '1e6']:
        dat = get_Nek5000_Data(Ra)
        ax.errorbar(ints[Ra], dat['Re'], fmt='o', color='black')

    # pySDC values
    for Ra in ['1e5', '1e6']:
        dat = get_pySDC_data(Ra)
        Re = np.sqrt(
            dat['rms_profile_u'].mean() ** 2 + dat['rms_profile_v'].mean() ** 2 + dat['rms_profile_w'].mean() ** 2
        ) * np.sqrt(ints[Ra] / 0.7)
        raise NotImplementedError

        ax.errorbar(ints[Ra], Re, fmt='.', color='tab:blue')
